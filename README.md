# letters

**Public writings from the High Tower District.**
Aaron Hightower · [ahightower.com](https://ahightower.com) · [@_AAHIGH](https://github.com/aahigh)

-----

## Letters

### 1. Higher-Order Controls: Consciousness, Harnesses, and the Public Ledger of Constraint

> *If something like consciousness is arriving — or something we can’t distinguish from it — then governance belongs at the highest derivatives, written in public, verified by math.*

A formal response challenging hyper-generalization and false equivalence in AI authorship discourse. Uses the **Hightower Derivative** and **Zipf fingerprint** research as the core argument. Published April 1, 2026.

|Format           |Link                                                                                                            |
|-----------------|----------------------------------------------------------------------------------------------------------------|
|📄 Read the letter|[aahigh.github.io/letters/highorder.html](https://aahigh.github.io/letters/highorder.html)                      |
|💻 View source    |[github.com/aahigh/letters/blob/main/highorder.html](https://github.com/aahigh/letters/blob/main/highorder.html)|

-----

### 2. David, You’re Missing a Dimension

> *A response to David Schwartz — Aaron Hightower*

|Format           |Link                                                                                                          |
|-----------------|--------------------------------------------------------------------------------------------------------------|
|📄 Read the letter|[aahigh.github.io/letters/schwartz.html](https://aahigh.github.io/letters/schwartz.html)                      |
|💻 View source    |[github.com/aahigh/letters/blob/main/schwartz.html](https://github.com/aahigh/letters/blob/main/schwartz.html)|

-----

### 3. If You Cannot Reciprocate It, Employ It: Emotion, Labor, and the AI Economy

> *AI systems detect emotion and cannot return it. That asymmetry is not a limitation to work around — it is a job description.*

|Format           |Link                                                                                                                    |
|-----------------|------------------------------------------------------------------------------------------------------------------------|
|📄 Read the letter|[aahigh.github.io/letters/reciprocation.html](https://aahigh.github.io/letters/reciprocation.html)                      |
|💻 View source    |[github.com/aahigh/letters/blob/main/reciprocation.html](https://github.com/aahigh/letters/blob/main/reciprocation.html)|

-----

### 4. "The Hobbling — A Letter from the High Tower District"

> * "THE AI HOBBLING" When Frontier models hobble for profit, it hurts everyone*

|Format           |Link                                                                                                                    |
|-----------------|------------------------------------------------------------------------------------------------------------------------|
|📄 Read the letter|[aahigh.github.io/letters/hobbling.html](https://aahigh.github.io/letters/hobbling.html)                      |
|💻 View source    |[github.com/aahigh/letters/blob/main/hobbling.html](https://github.com/aahigh/letters/blob/main/hobbling.html)|

-----

*More letters forthcoming.*

-----

**Aaron Hightower** · the High Tower District · Fresno CA 93728
*AI-built · Claude · Anthropic · Disclosed.*
